...
class imgbrowse {
  ...
  protected $root = '/goodarch2u/upload/ckeditor/';
  ...
  public function getMenuImgs() {
    ...
    foreach($obdr as $fileobj) {
      ...
      if($fileobj->isFile() && in_array($fileobj->getExtension(), $this->imgext)) $re['imgs'] .= '<span><img src="/' . $this->root . $this->imgdr . $name .'" alt="'. $name .'" height="50" />'. $name .'</span>';
