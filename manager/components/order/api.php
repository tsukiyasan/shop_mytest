...
switch ($task) {
	...
	case "memberSearch":
		...
		$params = global_get_param($_POST, 'params', null, 0, 1);

		$memberSql = "SELECT * FROM members WHERE 1";
		if (!($params['erpid'] == "")) {
			$memberSql .= " AND members.ERPID = '{$params['erpid']}'";
		}
		if (!($params['name'] == "")) {
			$memberSql .= " AND members.name LIKE N'%{$params['name']}%'";
		}
		if (!($params['mobile'] == "")) {
			$memberSql .= " AND members.mobile = '{$params['mobile']}'";
		}
		if (!($params['sid'] == "")) {
			$memberSql .= " AND members.sid = '{$params['sid']}'";
		}
		if (!($params['email'] == "")) {
			$memberSql .= " AND members.email = '{$params['email']}'";
		}
		$db->setQuery($memberSql);
		$members = $db->loadRowList();
		$returnData['data'] = $members;
...
function tracepage()
{
	...
	if (!empty($traceNumber)) {
		$traceUrl = "https://www.tracking.my/externalcall?style=iframebox&lang=en&tracking_no=" . $traceNumber;
	}
...
function showlist()
{
	...
	$orderMode2 = global_get_param($_REQUEST, 'orderMode2', null);
	...
	if (isset($orderMode2) && $orderMode2 != -1) { //orderMode

		if ($orderMode2 == '1') {
			$where_str .= " AND A.orderMode <> 'twcart'";
		} else if($orderMode2 == '2'){
			$where_str .= " AND A.orderMode = 'twcart'";
		}
...
function showdetail()
{
	...
	$sql = "SELECT _m.*,p.`".$sql_str."` as p_name 
	FROM orders as _m 
	LEFT JOIN pubcode as p on p.codeValue = _m.status 
	WHERE p.codeKinds = 'bill' and _m.id = '$id'";
	$db->setQuery($sql);
	$r = $db->loadRow();
	...
	if ($data['payTypeCode'] == '6') {
		...
		$json_str = getFieldValue("SELECT response FROM public_bank_log WHERE status = '1' and orderNum = '$orderNum'","response");
		$json_arr = json_decode($json_str);
		if(!empty($json_arr)){
			$card_num = $json_arr->req_card_number;
			$last4digitpan = substr($card_num,-4);
		}
...
function updatepage()
{
	...
	$member_id = $tid = $order_arr['memberid'];
	$memberid = $tid = $order_arr['memberid'];
	$msql = "SELECT * FROM members where id = '$member_id'";
	$db->setQuery($msql);
	$member_detail = $md = $db->loadRow();
	$m_name = $member_detail['name'];
	$m_mobile = $member_detail['mobile'];
	...
	if ($status == 1 || $status == 3 || $status == 4) {
		$orderId = $order_arr['id'];
        $orderNum = $order_arr['orderNum'];
        $memberId = $order_arr['memberid'];
        $buyDate = $order_arr['buyDate'];
		$orderMode = $order_arr['orderMode'];
        $totalAmt = $order_arr['totalAmt'] - $order_arr['m_discount'] - $order_arr['cb_use_points'] - $order_arr['use_points'];
		...
		if ($order_arr['orderMode'] == 'addMember') {
			...
			$om = getFieldValue("SELECT onlyMember FROM members WHERE id = '{$order_arr['memberid']}'","onlyMember");
			if($om == '1'){
				$target = '0';
			}else{
				$target = '1';
			}
			$csql = "SELECT * FROM toMLM_log WHERE orderNum = '{$order_arr['orderNum']}' and target_status in (1,3,4) and has_export = '1'";
			$db->setQuery($csql);
			$chklog = $db->loadRowList();
			if(empty($chklog)){
				
				$has_export = '1';
				export_tomlm($order_arr['memberid'],$target);
			}else{
				$has_export = '0';
			}
		...
		if ($order_arr['orderMode'] == 'updateMember') {
			$now = date('Y-m') . '%';
			$nowtime = date('Y-m-d');
			...
			$umsql = "UPDATE members set onlyMember = '0',exMember = '1',exTime = '$nowtime' where id ='$memberid'";
			$db->setQuery($umsql);
			$db->query();

			$umsql2 = "UPDATE members set payDate='$today' where id ='$memberid' and payDate = ''";
			$db->setQuery($umsql2);
			$db->query();
			...
			// $body = "會員 $mb_no $m_name 已由會員升級上傳證件照片，請盡速確認。";
	...
	if ($status == 1 || $status == 3 || $status == 4) {
		$get_point_url = "http://192.168.7.46/money_bank_my_test/public/api/front_orders/calc_points/" . $order_arr['orderNum'];
		$results = file_get_contents($get_point_url);
		$get_point_url2 = "http://192.168.7.46/money_bank_my_test/public/api/front_orders/calc_cb_points/" . $order_arr['orderNum'];
	...
	record_tomlm($status,$order_arr,$has_export);
...
function record_tomlm($status,$order,$has_export){
	global $db, $conf_user;
	$mid = loginChk(); //使用者
	...
	$data['context'] = $order_json;
	$data['muser'] = $mid;
	$data['has_export'] = $has_export;
	$sql = dbInsert('toMLM_log',$data);
	$db->setQuery($sql);
	$db->query();
