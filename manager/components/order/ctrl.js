    ...
    function (
      ...
      if (param) {
        ...
        my.params = {
          ...
          cdate: {
            startCDate: param.cdate
              ? !param.cdate.startCDate
                ? null
                : param.cdate.startCDate
              : null,
            endCDate: param.cdate
              ? !param.cdate.endCDate
                ? null
                : param.cdate.endCDate
              : null,
          },
        ...
        my.isIndeterminate = function () {
          return (
            my.selected.length !== 0 &&
            my.selected.length !== my.data_list.length - cancelcnt
          );
        };
        my.isChecked = function () {
          return (
            my.selected.length === my.data_list.length - cancelcnt &&
            my.data_list.length > 0
          );
        };
        ...
        my.export = function () {
          ...
          $timeout(function () {
            location.href =
              "/manager/components/order/api.php?task=upd_exportChk&idstr=" +
              idstr;
            //my.list();
          }, 100);
        ...
        my.operateChg = function (v) {
          if (
            ((v == 1 || v == 2) && my.funcPerm.U == "true") ||
            (v == 3 && my.funcPerm.D == "true")
          ) {
            if (v) {
              if (my.selected.length > 0) {
                ...
                if (txt) {
                  alertify
                    ...
                    .setHeader(
                      "<i class='fa fa-info-circle'></i> " +
                        $translate.instant("lg_main.batch_opt_msg")
                    )
                    .set({
                      labels: {
                        ok: $filter("translate")("lg_main.yes"),
                        cancel: $filter("translate")("lg_main.no"),
                      },
                    })
                  ...
                  if (v == 4) {
                    window.open(
                      "components/order/print.php?v=4&idStr=" + idstr
                    );
                  } else if (v == 5) {
                    window.open(
                      "components/order/print.php?v=5&idStr=" + idstr
                    );
                  } else {
                    var param = {
                      id: listCB,
        ...
        if (my.params.status) {
          if (my.params.status == -1) {
            ...
            my.orderTypeStr =
              $rootScope.pubcodeArr.bill[my.params.status]["name"];
          }
        ...
        my.exportChkChg = function (v) {
          ...
          if (v == -1) {
            ...
            my.exportChk =
              v == 1
                ? $translate.instant("lg_order.order_exportYes")
                : $translate.instant("lg_order.order_exportNo");
          }
          my.list();
        };
        ...
        my.exportProductReport = function (productId) {
          ...
          my.selectModalInstance.result.then(function (date) {
        ...
        my.orderModeChg2 = function (v) {
          my.params.orderMode2 = v;
          if (v == -1) {
            my.orderMode = $translate.instant("lg_order.order_orderMode"); //訂單類別
          } else {
            var tmpStr = "";
            if (v == 1) {
              tmpStr = $translate.instant("lg_order.order_cart"); //一般
            } else if (v == 2) {
              tmpStr = $translate.instant("lg_order.order_twcart"); //台灣
            }
            my.orderMode = tmpStr;
          }
          my.list();
    ...
    function (
      ...
      if (param) {
              ...
              $scope.$watch("ctrl.order.status", function (newVal) {
                ...
                if (my.pcode["bill"] && oldVal != newVal) {
                  ...
                  if (my.order.payTypeCode == 3 && oldVal == 0) {
                    ...
                    if (
                      (newVal == 2 && my.order.payTypeCode == 1) ||
                      (newVal == 1 && my.order.payTypeCode == 3)
                    ) {
                      cname =
                        $rootScope.pubcodeArr["bill"][newVal]["codeName_chs"];
                    }
                    ...
                    if (my.funcPerm.U == "true") {
                      alertify
                        .confirm(
                          msgStyle(
                            $translate.instant("lg_order.order_statusChg") +
                              cname +
                              "?"
                          )
                        )
                        ...
                        .set({
                          labels: {
                            ok: $translate.instant("lg_main.ok"),
                            cancel: $translate.instant("lg_main.cancel"),
                          },
                        })
                        .set("onok", function (closeEvent) {
        ...
        my.cancel_order = function () {
          if (my.funcPerm.U == "true") {
            alertify
              .confirm(msgStyle($translate.instant("lg_order.order_msg2")))
              .setHeader(
                "<i class='fa fa-info-circle'></i> " +
                  $translate.instant("lg_main.batch_opt_msg")
              )
              ...
              .set("onok", function (closeEvent) {
                CRUD.update({ task: "cancel", id: my.order.id }, "POST").then(
                  function (res) {
                    if (res.status == 1) {
                      success(res.msg);
                      $route.reload();
        ...
        my.return_order = function () {
          if (my.funcPerm.U == "true") {
            alertify
              .confirm(msgStyle($translate.instant("lg_order.order_msg3")))
              .setHeader(
                "<i class='fa fa-info-circle'></i> " +
                  $translate.instant("lg_main.batch_opt_msg")
              )
              ...
              .set("onok", function (closeEvent) {
                CRUD.update({ task: "return", id: my.order.id }, "POST").then(
                  function (res) {
                    if (res.status == 1) {
                      success(res.msg);
                      $route.reload();
      ...
      my.memberModal = {
        ...
        search: {
          ...
          btn: function () {
            ...
            CRUD.setUrl("components/order/api.php");
            CRUD.detail(
              { task: "memberSearch", params: vm.params },
              "POST"
            ).then(function (res) {
              console.log(res);
              my.memberModal.select.init();
        ...
        getMember: function () {
          if (!(this.select.member == null)) {
            my.order.member = JSON.parse(JSON.stringify(this.select.member));
          }
        },
      };
      my.order = {
