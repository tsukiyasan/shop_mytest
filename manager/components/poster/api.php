	...
	if ($mode == 'all') {
		foreach ($r as $key => $row) {
			...
			if(!empty($row['start_date'])){
				$info['start_date'] = date('Y-m-d',strtotime($row['start_date']));
			}
			if(!empty($row['end_date'])){
				$info['end_date'] = date('Y-m-d',strtotime($row['end_date']));
			}
		...
		for ($i = $from; $i < min($end, $cnt); $i++) {
			...
			$info['id'] = $r[$i]['id'];
			$info['name'] = $r[$i]['name'];
			if(!empty($r[$i]['start_date'])){
				$info['start_date'] = date('Y-m-d',strtotime($r[$i]['start_date']));
			}
			if(!empty($r[$i]['end_date'])){
				$info['end_date'] = date('Y-m-d',strtotime($r[$i]['end_date']));
			}
...
function showdetail()
{
	...
	$db->setQuery($sql);
	$poster_arr = $db->loadRow();
	$posterList = array();
	if (count($poster_arr) > 0) {
		$info = array();
		$info['id'] = $id;
		$info['name'] = $poster_arr['name'];
		$info['publish'] = intval($poster_arr['publish']);
		$info['content'] = $poster_arr['content'];
		$info['odring'] = $poster_arr['odring'];
		$info['linktype'] = $poster_arr['linktype'];
		$info['linkurl'] = $poster_arr['linkurl'];
		if (!empty($poster_arr['start_date'])) {
			$info['start_date'] = $poster_arr['start_date'];
		}
		if (!empty($poster_arr['end_date'])) {
			$info['end_date'] = $poster_arr['end_date'];
		}
...
function updatepage()
{
	global $db, $conf_user, $tablename, $conf_poster;
	$arrJson = array();
	...
	$dataArr['start_date']	= global_get_param($_REQUEST, 'start_date', null);
	$dataArr['end_date']	= global_get_param($_REQUEST, 'end_date', null);
	...
	if (!$db->query()) {
		...
		$fid = getFieldValue("SELECT LAST_INSERT_ID()", "LAST_INSERT_ID()");
		$id = $dataArr['id'] ? $dataArr['id'] : $fid;
		...
		if (count($img) > 0) {
			foreach ($img as $key => $value) {
				if ($value) {
					$path = $id . "_" . $key . ".jpg";
					imgupd($value, $conf_poster . $path, $tablename, $id, $key);
				}
			}
		}
...
function deletepage()
{
	global $db, $conf_user, $conf_poster, $tablename, $conf_dir_path;
	$arrJson = array();
