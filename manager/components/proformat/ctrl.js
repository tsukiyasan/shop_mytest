app.controller('proformat_list',['$rootScope','$scope','$http','$location','$route','$routeParams','$translate','CRUD','urlCtrl','$filter',function($rootScope,$scope, $http, $location,$route,$routeParams,$translate,CRUD,urlCtrl,$filter) {
	var my=this;
	var param=urlCtrl.deaes($location.hash());//解碼網址hash
	if(param){
		//api位置
		CRUD.setUrl("components/proformat/api.php");
		
		my.funcPerm = $rootScope.funclist['proformat'];
		...
		my.params = {
				...
				level: !param.level ? 1 : param.level,
		...
		//新增的hash
		my.odrhash=urlCtrl.enaes({component:'proformat',p:param});
			...
			CRUD.list(my.params, "GET").then(function(res){
				if(res.data.status == 1) {
					...
					angular.forEach(res.data.data,function(v,key){
						res.data.data[key].param=urlCtrl.enaes({belongid:v.id,level:parseInt(v.level)+parseInt(1),page:1,p:my.params});
...
}]).controller('proformat_dir',['$rootScope','$scope','$http','$location','$route','$translate','CRUD','urlCtrl',function($rootScope,$scope, $http, $location,$route,$translate,CRUD,urlCtrl) {
	var my=this;
	var param=urlCtrl.deaes($location.hash());//解碼網址hash
	my.textList = [];
	my.nameList = [];
	...
	my.submit=function(){
		...
		//var name =  my.proformat_dtl.name;		
		//if(!name)
		//{
		//	error($translate.instant('lg_proformat.name')+$translate.instant('lg_main.empty'));
		//	return false;
		//}
		...
		var params = {
			...
			name:my.proformat_dtl.name,
			publish:my.proformat_dtl.publish
		};
		...
		if( (!param.id && my.funcPerm.C == 'true') || (param.id && my.funcPerm.U == 'true') ) 
		{
			...
			CRUD.update(params, "POST").then(function(res){
				if(res.status == 1) {
					success(res.msg);
					urlCtrl.go("proformat_list",param);
				}
			});
...
}]).controller('proformat_page',['$rootScope','$scope','$http','$location','$route','$routeParams','$translate','urlCtrl','CRUD',function($rootScope,$scope, $http, $location,$route,$routeParams,$translate,urlCtrl,CRUD) {
	var my=this;
	var param=urlCtrl.deaes($location.hash());//解碼網址hash
	my.content_list=[];
	var uploader=[];
			...
			my.detail = function() {
				CRUD.detail(params, "GET").then(function(res){
					if(res.status == 1) {
						...
						my.proformat_dtl=res.data;
						my.nameList=res.nameList;
						if(my.proformat_dtl.var1 == 'null')
						{
							my.proformat_dtl.var1 = '';
						}
						
						my.proformat_dtl.url=res.data.url+urlCtrl.enaes({id:res.data.id});
						//seteditor('proformat_content');
					}
				});
			}
			my.detail();
		...
		if( (!param.id && my.funcPerm.C == 'true') || (param.id && my.funcPerm.U == 'true') ) 
		{
			CRUD.update(params, "POST").then(function(res){
				if(res.status == 1) {
					...
					success(res.msg);
					urlCtrl.go("proformat_list",param.p);
				}
			});
		}
