...
function ComputeDesActve($allcart, $cart, $act, $actRangePCode, $discount, $discountFree, $activeProChk = false)
{
	...
	if (count($cart) > 0) {
		...
		switch ($actTypePCode) {
			case "2":
				...
				if ($act['var01'] > "0") {
					...
					$pa = bcsub($allcart['totalAmt'], $cb_use_points, 2);
					$freeCnt = (int)($pa / $act['var01']);
				}
		...
		if (
			$activePlanid != "1" && $activePlanid != "2" && $activePlanid != "3"
			&& $effectiveTime > 0 && $act['limitChk']
			&& ($effectiveTime + $historyEffectiveTimes) > $act['limitCnt']
		) {
			$cartChk = false;
...
function CartProductInfo2($cart, $activeChk = null, $activeProChk = null)
{
	...
	if (count($cart) == 0 && (!$_SESSION[$conf_user]['activeBundleCart'] || count($_SESSION[$conf_user]['activeBundleCart']) == 0)) {
		if ($mode == 'amtpro' || $mode == 'freepro' || $mode == 'e3pro') {
			...
			JsonEnd(array("status" => 0, "msg" => _EWAYS_CART_EMPTY, "cnt" => 0, 'err' => $_SESSION[$conf_user]['cart_list']));
		}
	}
	...
	if ($mode == 'cart' || $mode == 'event' || $mode == 'twcart') {
		$tmpArr = array();
		foreach ($cart as $key => $row) {
			$pid = explode("|||", $key);

			if (($mode == 'event' && $pid[2] == 'event') || ($mode == 'cart' && $pid[2] != 'event') || ($mode == 'twcart' && $pid[2] != 'event')) {
				$tmpArr[$key] = $row;
			}
		}
		$cart = $tmpArr;
	...
	if ($mode == "cart") {
		...
		$sql = "select A.id,A.name,A.highAmt,A.siteAmt,A.pv,A.bv,ccv,A.rate_note,'0' as bonus,A.nccbChk,B.ptid,C.belongid as rootid,A.bundleaddChk,A.bundleadd,A.is_dlvrfree from $tablename A,protype B,producttype C where A.id in (" . implode(",", $pidArr) . ") AND A.publish=1 AND B.pid = A.id AND C.id = B.ptid and C.belongid != 'root' order by C.belongid desc";
	} else if ($mode == "bonus") {
		$discountAble = false;
...
function toMLM($oid, $paid_money, $is_zero = false)
{ //插入訂單資訊
	...
	if (!empty($o)) {
		...
		$d['send_method'] = $send_method; //目前只有寄送
		...
		$d['memo'] = $o['dlvrNote'];
		$d['io_kind'] = 1;
		if ($o['orderMode'] == 'addMember' || $o['orderMode'] == 'updateMember') {
			//入會訂單為1
			$order_kind = 3;
		} else {
			//網路訂單為3
		...
		$d['total_pv'] = $o['pv'];
		$atotal2 = bcsub($o['totalAmt'], $o['m_discount'], 2);
		...
		$d['total_paid_money'] = $paid_money;
		// if($is_zero == true){
		$d['order_money'] = bcsub($o['totalAmt'], $o['dlvrFee'], 2);
		// }else{
		// $d['order_money'] = bcsub($d['total_money'], $o['dlvrFee'], 2);
		// }
	...
	$order_mSql = "SELECT * FROM order_m WHERE ord_no = '$orderNum'";
	$db2->setQuery($order_mSql);
	$existOrder = $db2->loadRowList();
	// JsonEnd($o);
	if (empty($existOrder)) {
...
function toMLMd($o, $ord_no, $mb_no, $paid_money)
{
	...
	if (count($orderdtl) > 0) {
		...
		foreach ($orderdtl as $each) {
			...
			if ($each['unitAmt'] == 0 || $each['unitAmt'] == '0.00') { //贈品判斷
				...
				$gd['prod_no'] = $pro_code;
				$gd['prod_unit'] = '個';
				$gd['qty'] = $each['quantity'];
				$gd['prod_kind'] = '1';
				$gd['update_user'] = 'web_auto';
				$gd['data_timestamp'] = strtotime('now');
				$gd['update_state'] = '1';
				$gd['add_kind'] = '0';
				$sql2 = dbInsert('order_gift', $gd);
				$db2->setQuery($sql2);
				$db2->query();

				$ogd['ord_no'] = $ord_no;
				$ogd['seq_no'] = $k;
				$ogd['gift_no'] = $pro_code;
				$ogd['prod_no'] = $pro_code;
				$ogd['prod_unit'] = '個';
				$ogd['qty'] = $each['quantity'];
				$ogd['prod_kind'] = '1';
				$ogd['update_user'] = 'web_auto';
				$ogd['data_timestamp'] = strtotime('now');
				$ogd['update_state'] = '1';
				$sql = dbInsert('order_gift_d', $ogd);
				$db2->setQuery($sql);
				$db2->query();
				$k++;
			} else {
				...
				$od['unit_bv'] = bcdiv($each['bv'], $each['quantity'], 2);
				// $od['unit_bv'] = $o['back_value'];
				$od['sub_bv'] = $each['bv'];

				$od['unit_cv'] = bcdiv($each['unitCcv'], $each['quantity'], 2);
				$od['sub_cv'] = $each['unitCcv'];
				...
				$odd['ord_no'] = $ord_no;
				$odd['seq_no'] = $i;
				$odd['prod_no'] = $pro_code;
				$odd['detail_prod_no'] = $pro_code;
				$odd['qty'] = $each['quantity'];
				$odd['prod_kind'] = '1';
				$odd['update_user'] = 'web_auto';
				$odd['data_timestamp'] = strtotime('now');
				$odd['remark'] = '';
				$odd['update_state'] = '1';
				$sql = dbInsert('order_dd', $odd);
				$db2->setQuery($sql);
				$db2->query();
...
function paid($o, $ord_no, $mb_no, $paid_money)
{
	...
	$p['pdate'] = date('Y-m-d');
	$pay_way = '1';
	if ($o['payType'] == '6') { //信用卡 20
		$pay_way = '20';
	} else if ($o['payType'] == '2' || $o['payType'] == '5') { //匯款 ATM 3
		$pay_way = '3';
	}
	$p['paid_way'] = $pay_way; //付款方式
	$p['mb_no'] = $mb_no;
	...
	$updateSql = dbUpdate('order_m', $updateData, "ord_no ='" . $ord_no . "'");
	$db2->setQuery($updateSql);
	$db2->query();
}
...
function get_new_ordno($temp_id)
{ //add by Dale 20120323
...
/**
 * 會員匯入傳銷
 * @param	$tid	members.id
 * @param	$target	經銷商(1),會員(0)
 */
...
function chk_cb($uid = null)
{
	...
	$mb_name = getFieldValue("select name from members where id='$uid'", "name");
	...
	//建立新的會員等級
	$msql = "SELECT * FROM member_lv WHERE mb_no = '$mb_no'";
	$db3->setQuery($msql);
	$m = $db3->loadRow();
	if (empty($m)) {
		$mdata = array();
		$mdata['mb_no'] = $mb_no;
		$mdata['mb_name'] = $mb_name;
		$mdata['lv'] = '1';
		$mdata['total'] = '1';
		$mdata['t_total'] = '1';
		$mdata['l_total'] = '1';
		$dsql = dbInsert('member_lv', $mdata);
		$db3->setQuery($dsql);
		$db3->query();
	}
}
...
function get_cash_back($uid = 0, $expiry_data = '')
{
	global $db3;
	//回傳資料
	$returnData = array(
	...
	$u_data = get_user_info_m($uid);
	$mb_no = $u_data['mb_no'];

	//回饋點資料
	$sql = "SELECT cash_back.point, cash_back.kind, cash_back.expiry_date, cash_back.note,";
	$sql .= " 1 as configID, 10 as use_percent, 10 as give_percent";
	$sql .= " FROM cash_back";
	$sql .= " WHERE cash_back.mb_no = '$mb_no' AND cash_back.is_invalid = '0'";
	if (!($expiry_data == '')) {
		$sql .= " AND cash_back.expiry_date > '$expiry_data'";
	}
	$db3->setQuery($sql);
	$cbpointData = $db3->loadRowList();

	if (!empty($cbpointData)) {
		$maxFormat = array(
			'point' => 0,
			'use_percent' => 0,
			'give_percent' => 0
		...
		foreach ($cbpointData as $_cbData) {
			//到期日期
			$expiryTime = strtotime($_cbData['expiry_date']);
			if (!isset($returnData['data'][$expiryTime])) {
				$returnData['data'][$expiryTime] = array();
			}
			//設定檔代號
			$configID = $_cbData['configID'];
			$_list = (isset($returnData['data'][$expiryTime][$configID])) ? $returnData['data'][$expiryTime][$configID] : $listFormat;

			//明細資料依照kind作分類
			if (!isset($_list['data'][$_cbData['kind']])) {
				$_list['data'][$_cbData['kind']] = array();
			}
			array_push($_list['data'][$_cbData['kind']], $_cbData);

			$_max = (isset($returnData['max'][$configID])) ? $returnData['max'][$configID] : $maxFormat;
			//使用上限%
			$_max['use_percent'] = $_cbData['use_percent'];
			//不知道什麼%
			$_max['give_percent'] = $_cbData['give_percent'];

			switch ($_cbData['kind']) {
					//可用
				case "0":
					$_list['point'] += $_cbData['point'];
					$_max['point'] += $_cbData['point'];
					$returnData['point'] += $_cbData['point'];
					break;
					//已用
				case "1":
					$_list['usedPoint'] += $_cbData['point'];
					$_max['point'] -= $_cbData['point'];
					$returnData['point'] -= $_cbData['point'];
					break;
			}
			$returnData['data'][$expiryTime][$configID] = $_list;
			$returnData['max'][$configID] = $_max;
		}
	}

	//若可用點數小於0以0計算(正常不會發生,不過還是作一下防呆)
	$returnData['point'] = $returnData['point'] < 0 ? 0 : $returnData['point'];
	//print_r($returnData);
	return $returnData;
}
 ...
 * 取得活動回饋點相關資料
 * @param $uid			UID
 * @param $expiry_data	有效日期
 * @param $enable		方案啟用/停用
 * @param $acbc_id		方案ID
 * @return array(
 * 		'point',
 * 		'data',
 ...
 * )
 */
function get_act_cash_back($uid = 0, $expiry_data = '', $enable = '', $acbc_id = "")
{
	global $db3;
	//回傳資料
	$returnData = array(
	...
	$mb_no = $u_data['mb_no'];
	//當下身分(會員/經銷商)
	$onlyMember = $u_data['om'];

	//回饋點資料
	$sql = "SELECT act_cash_back.*,";
	$sql .= " act_cash_back_config.*";
	$sql .= " FROM act_cash_back";
	$sql .= " LEFT JOIN act_cash_back_config ON act_cash_back_config.id = act_cash_back.acbc_id";
	$sql .= " WHERE act_cash_back.mb_no = '$mb_no' AND act_cash_back.is_invalid = '0'";
	//當下身分(會員/經銷商)的點數
	$sql .= " AND act_cash_back.onlymember = '$onlyMember'";
	if (!($enable == '')) {
		//方案啟用
		$sql .= " AND act_cash_back_config.enable = '$enable'";
	}
	//方案適用對象
	$sql .= " AND act_cash_back_config.target = '$onlyMember'";
	if (!($expiry_data == '')) {
		$sql .= " AND act_cash_back.expiry_date > '$expiry_data'";
	}
	//方案ID
	if (!($acbc_id == '')) {
		$sql .= " AND act_cash_back_config.id = '$acbc_id'";
	}
	$db3->setQuery($sql);
	$cbpointData = $db3->loadRowList();
...
function cnum_format($num)
{
	return number_format($num, 1);
}

function c_round($num, $scaleVal)
{
	$t1 = bcmul($num, 2, $scaleVal); //先將數字*2
	$t2 = round($t1, 1); //四捨五入到第一位
	$t3 = bcdiv($t2, 2, $scaleVal); //將數字除以2還原
	return $t3;
}
